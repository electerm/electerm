...
| --- | --- | --- | --- | --- |
| 准备 | Step 0 基线快照 | — | — | ✅ 已完成 |
| Layer 1 | Step 1 新增 tokens | 1 | 🟢 | ✅ **已完成**（b6ecc646；v4 扩展 c8d717a4） |
| Layer 2 | Step 2–8 统一层 | 7 | 🟢🟢🟢🟡🟠🟠🟢🟢 | ✅ **全部完成**（2/3/4/5a/5b/6/7a/7b/8） |
| 单测 | Step 9 新增 4 个 spec | 1 | 🟢 | ✅ **已完成**（a32142a1，460 test；执行顺序与 Step 10 对调，见 §6.10） |
| Layer 3 | Step 10–26 逐区域精修 | 17 | 🟢（2 个 🟠） | ✅ **全部完成**（含 v4 四要素 + 首页 hero） |
| 可选 | Step 27 方案 B | 1 | 🟢 | ✅ **已完成**（.theme-light 阴影分档） |

> **执行状态（2026-09-24）**：Step 1–27 全部落地，每步独立提交可单独 revert。
> 最终验证：compile 通过、**460/460 契约单测**（含 4 个新 CSS 守卫 + 反向验证）、lint 通过。
> 完整提交清单见 git log（Step 9 说明：因单测 3 断言零硬编码，与 Step 10 执行顺序对调）。
