...
describe('sftp file transfer', function () {
  it('should open window and basic sftp works', async function () {
    ...
    await client.rightClick('.session-current .file-list.local .virtual-file', 10, 100)
    await delay(3300)
    log('add folder')
    ...
    await client.rightClick('.session-current .file-list.remote .virtual-file', 10, 100)
    await delay(200)
    await client.click('.ant-dropdown .anticon-folder-add')
    await delay(200)
